...
```
python3 process_images.py image1.tif image2.tif
```
* You can add optional parameters to specify the weights file and/or save the intermediate pictures:
```
python3 process_images.py [--weights_file <filename>] [--write_nn_mask] [--write_threshold_mask] [--write_area_filtered]
```
### Simple Run with Mac GUI
* You can run a simple process_images.py command for all tif in a folder along with selecting a weights file and save location with the command, please ensure python is up to date \:
```
python3 PPGUI.py
```
...
```
python3 calculate_iou.py image1.tif image2.tif
```

* For the accuracy classification script, run the command:
```
python3 classification_acc.py image1.tif image2.tif
```

* For labeling images, run the command:
```
pytho3n label_image.py image1-area_filtered.tif image2-area_filtered.tif 
```
